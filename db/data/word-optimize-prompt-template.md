...
7. 部首区汉字乱码（康熙部首/部首补充区 U+2E80–U+2FDF 的字符，如 ⻜/⾏/⻝/⼀/⾜/⽣/⿅）：源数据
   hongbaoshu_meanings.json 曾大量混入（如「⻜机」「旅⾏」「⻝物」「⼀⼝」），现源文件与
   DB 均已清理为标准简体字。若输入 senses 或旧义项里再见到这类字符，输出时必须改写为对应
   简体（飞机/旅行/食物/一口…），不得保留、不得原样输出。
8. 义项内混用半角逗号（「n.注意，观察,话语」）与用 '&' 拼接词性（「vt.& vi.评论」）：统一改为
   全角标点；多个词性用 '/' 分隔并后接空格（如「vt./vi. 评论，觉察」）。
...
3. 检查错别字（例：「可诣达性」应为「可达性」）与部首区乱码字符（⻜/⾏/⻝/⼀ 等，见上方第 7 条），
   输出前逐条通读、一律输出标准简体。
